...
def main():
    ...
    # Log status change
    audit_service.log_status_changed(invoice, new_status="PAID", old_status=old_status, user_id="admin")

    # 6. Check logs (in current session)
    print("\n4. Checking audit logs in current session:")
    logs = audit_service.get_logs(invoice_id=invoice.id)
    for log in logs:
        print(f"   [{log.timestamp.strftime('%H:%M:%S')}] {log.action}: {log.new_value or log.notes}")
