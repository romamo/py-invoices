...
@pytest.fixture
def invoice_repo(factory):
    return factory.create_invoice_repository()

@pytest.fixture
def client_repo(factory):
    return factory.create_client_repository()

@pytest.fixture
def base_invoice(invoice_repo, client_repo):
    from pydantic_invoices.schemas import ClientCreate
    client = client_repo.create(ClientCreate(name="Test Client"))
...
def test_strict_state_machine(invoice_repo, base_invoice):
...
def test_modification_restriction(invoice_repo, base_invoice):
...
def test_credit_note_creation(invoice_repo, base_invoice):
