...
def test_clients_create_and_list():
    # 1. List empty
    ...
    # For unit testing CLI with memory backend, we might need to mock or use a persistent backend fixture if we want continuity.
    # However, let's just check the create output confirmed creation.
    # If we wanted to test persistence, we'd need to mock the repository or use a file-based sqlite for tests.
    pass

def test_invoices_create_fail_no_client():
    result = runner.invoke(app, ["invoices", "create", "--amount", "100", "--description", "Desc", "--backend", "memory"])
    assert result.exit_code == 1
    assert "Must provide --client-id or --client-name" in result.stdout

def test_invoices_help():
...
def test_invoices_pdf_generation_mock():
    # Mock test for PDF generation
    result = runner.invoke(app, ["invoices", "pdf", "999", "--company-name", "Test Co", "--company-address", "Test Addr", "--backend", "memory"])
    assert result.exit_code == 1
...
def test_invoices_html_generation_mock():
    # Similar mock test for HTML
    result = runner.invoke(app, ["invoices", "html", "999", "--company-name", "Test Co", "--company-address", "Test Addr", "--backend", "memory"])
    assert result.exit_code == 1
    assert "Invoice" in result.stdout
    assert "999" in result.stdout
    assert "not found" in result.stdout

def test_invoices_create_with_formats_mock(monkeypatch):
...
def test_invoices_create_autocreate_client():
...
def test_invoices_create_instant_ubl_validation(monkeypatch):
    from unittest.mock import MagicMock
