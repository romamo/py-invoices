...
def test_save_load_yaml_success(temp_storage, tmp_path):
    """Test saving and loading YAML when pyyaml is available."""
    # Ensure pyyaml is available/mocked if not installed
    try:
        import yaml
    except ImportError:
...
def test_missing_pyyaml_error(temp_storage):
...
def test_load_yaml_missing_lib_error(temp_storage, tmp_path):
...
def test_markdown_yaml_integration(temp_storage):
...
def test_markdown_fallback_json(temp_storage):
