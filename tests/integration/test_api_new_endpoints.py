...
def seed_data() -> None:
    ...
    if not note_repo.get_all():
        note_repo.create(
            PaymentNoteCreate.model_construct(
...
def test_payment_notes_api() -> None:
    # Default
...
def test_invoices_api_extended() -> None:
    ...
    _shared_factory.create_invoice_repository()
    ...
    # HTML
    # Note: HTML generation might fail if templates are missing or jinja2 not installed
...
def test_pdf_generation() -> None:
    ...
    if response.status_code == 501:
        # WeasyPrint missing
        assert (
            "ImportError" in response.json()["detail"] or "WeasyPrint" in response.json()["detail"]
        )
    else:
        assert response.status_code == 200
