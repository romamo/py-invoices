...
class CreditService:
    ...
    def create_credit_note(
                 ...
                 if 0 <= idx < len(original_invoice.lines):
                     ...
                     cn_lines.append(
                        InvoiceLineCreate(
                            ...
                            quantity=orig_line.quantity, # Usually negative? Or keep positive and rely on InvoiceType?
                            # Standard practice: Credit Note usually has positive amounts that effectively reduce balance
                            # But visuals are negative.
                            # Let's stick to positive quantities and amounts for now, 
                            # and let the "InvoiceType=CREDIT_NOTE" handle the accounting logic (subtraction).
        ...
        # For simplicity, we use the standard numbering but users can configure prefix in NumberingService
